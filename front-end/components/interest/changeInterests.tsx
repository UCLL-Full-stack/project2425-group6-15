...
const ChangeInterests: React.FC<ChangeInterestsProps> = ({ onClose }) => {
    ...
    const saveInterests = async () => {
        if (selectedInterests.length < 5) {
            alert(t("interests.select_minimum"));
            return;
        }
            ...
            <div className="w-full bg-white flex flex-col items-center py-5 px-2">
                {isAdmin && (
                    <div className="w-full flex flex-col items-center mb-5">
                        <input
                            type="text"
                            placeholder={t("interests.new_interest_name")}
                        ...
                        <input
                            type="text"
                            placeholder={t("interests.new_interest_description")}
                ...
                <input type="text" id="interestInput" placeholder={t("interests.search_placeholder")} className="w-10/12 bg-white border border-gray-300 text-gray-500 rounded-full px-3 py-1 m-1" />
                ...
                <button className={`self-end justify-self-end rounded-full w-32 h-8 border-gray-600 border ${selectedInterests.length > 4 ? 'hover:bg-slate-600 hover:text-white' : ''}`} onClick={saveInterests}>
                    ...
                    {selectedInterests.length > 4 && t("interests.save")}
                </button>
            </div>
        </>
    );
