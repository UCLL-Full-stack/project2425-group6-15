...
const PostOverviewPopup: React.FC<CreateNewPostPopupProps> = ({ onClose, postId }) => {
    ...
    const fetchPost = async () => {
        ...
        setPost(data);
        if (data.location) {
            const nearestAddress = await fetchNearestAddress(data.location.latitude, data.location.longitude);
    ...
    return (
        <div className="bg-black bg-opacity-50 z-[999] fixed top-0 left-0 w-full h-full flex justify-center items-center">
            <div className="bg-white px-4 py-2 flex flex-col gap-0 relative rounded-xl w-10/12 max-w-xl min-w-96">
                ...
                <p className="text-sm text-gray-400">By @{post?.creator?.firstName} {post?.creator?.lastName}</p>
                <div className="pt-4 grid grid-cols-2 gap-2">
                    <div>
                        ...
                        <div>
                            <MapContainerNoSSR center={[Number(post?.location?.latitude) || 0, Number(post?.location?.longitude) || 0]} zoom={13} style={{ height: "200px", width: "100%" }}>
                                <TileLayerNoSSR
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <div className="flex flex-col">
                            <p className="text-sm text-gray-400">{post?.location ? `${post.location.latitude}, ${post.location.longitude}` : ''}</p>
                ...
                <div className="w-full flex items-center justify-end">
                    <button className="flex items-center justify-center gap-2 bg-blue-500 text-white rounded-full px-4 py-2 transition-all duration-300 ease-in-out hover:bg-blue-600">
                        <Image src={checkmarkImg} alt="Checkmark Icon" width={20} height={20} />
                        Join
                    </button>
                </div>
            </div>
